...
export default function Home() {
  const guides = [
    {
      description: "所有しているモノ・欲しいモノを管理して、 \n好きなものにだけ囲まれた生活を目指そう！",
      url: "/stuff",
      linkText: "カテゴリーを見る",
  ...
  return (
    <main className="w-defaultWidth m-auto mt-16">
      ...
      <div className="flex flex-col gap-6">
        {guides.map((guide) => (
          <div key={guide.linkText} className="inline-flex flex-col items-center p-10 bg-gray border border-line rounded-lg">
            <p className="block mb-6 text-defaultText text-center whitespace-break-spaces">
              {guide.description}
            </p>
            <Link href={guide.url} className={`inline-block w-fit px-10 py-4 text-[1.4rem] font-bold rounded-full ${guide.linkColor}`}>
              {guide.linkText}
            </Link>
        </div>
        ))
